...
const historyFile = path.join(__dirname, 'speakmemo-history.json');
...
app.get('/api/stats', (req, res) => {
  try {
    ...
    data.forEach(entry => {
      const tipo = entry.type || 'desconhecido';
      const contexto = entry.context || 'sem-contexto';

      stats.porTipo[tipo] = (stats.porTipo[tipo] || 0) + 1;
      stats.porContexto[contexto] = (stats.porContexto[contexto] || 0) + 1;
      stats.porRotina[entry.routine || 'sem-rotina'] = (stats.porRotina[entry.routine || 'sem-rotina'] || 0) + 1;
    });
...
// WebSocket: envio de mensagens em tempo real
...
async function startConsumer() {
  ...
  ch.consume(QUEUE_NAME, (msg) => {
    if (msg !== null) {
      ...
      console.log("📝 Interação registrada no histórico");
